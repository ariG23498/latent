...
1. $p_{\theta}(x_{0:T})$ is the **joint reverse distribution** and $p_{\theta}(x_{t-1} | x_{t})$ is known as the **reverse diffusion kernel**
2. $\mu_{\theta}(x_{t}, t), \Sigma_{\theta}(x_{t}$, t) are the learnable parameters of the reverse distribution
...
If you have worked with other types of generative models before, then you might have seen something like equation eq.*(4)* in other types of generative models. But where? The answer is **Variational AutoEncoder**(VAE for short). Let's consider a single step of denoising. We assume a noisy image to be our latent, and once we have our latent variable we try and generate a less noisy image. Both the latent variable (more noisy image) and the generated image (less noisy image) come from a distribution. If we draw an analogy of this step to a VAE we can apply all the math onto each reverse diffusion process steps.
